// 本节实现只读和浅只读
// 组件接收到的props 对象应该是一个只读数据
// 例如组件接收到的props 对象应该是一个只读数据。这时就要用到接下来要讨论的readonly 函数，它能够将一个数据变成只读的：
...
function createReactive(obj: Data, isShallow = false, isReadonly = false) {
